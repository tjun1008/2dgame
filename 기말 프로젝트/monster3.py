...
class Monster3:
    ...
    def __init__(self):
        ...
        if n == 0:
            ...
            if m == 0:
                self.PAT_POSITIONS = [(30, 370), (230, 370)]
            elif m ==1:
                self.PAT_POSITIONS = [(490, 370), (730, 370)]
            else:
                self.PAT_POSITIONS = [(900, 370), (1100, 370)]

            

        if len(Monster3.images) == 0:
            Monster3.load_all_images()

        if n == 0:
            self.pos = (random.randint(300,1200),100)
        else:
            if m == 0:
                self.pos = (random.randint(30,230),370)
            elif m == 1:
                self.pos = (random.randint(490,730),370)
            else:
        ...
        self.images = Monster3.load_images(self.char)
        self.action = 'Idle'
    ...
    def move_to_target(self):
        ...
        if self.ball != None:
            ...
            if collides:
                ...
                if len(Monster3.cc) == 1:
                    self.action = 'Dead'
                    self.time = 0
                else:
                    self.ball = None
                    for (a, b) in Monster3.cc:
                        if (a, b) != (x, y):
                            self.action = 'Dead'
                            self.time = 0
                print(Monster3.cc)
        ...
        if Monster3.Over == True:
            self.remove()

        for (px, py) in self.PAT_POSITIONS:
            dsq = (px-x)**2 + (py- y)**2
            if dsq < Monster3.CHASE_DISTANCE_SQ ** 2:
                return BehaviorTree.SUCCESS
            else:
    ...
    def do_idle(self):
        ...
        if self.time >= Monster3.IDLE_INTERVAL:
            self.action = 'Walk'
            return BehaviorTree.FAIL
        return BehaviorTree.SUCCESS
    ...
    @staticmethod
    def load_all_images():
        Monster3.load_images('male')
        Monster3.load_images('female')
        # Monster3.font = gfw.font.load(gobj.RES_DIR + '/ENCR10B.TTF', 20)

    @staticmethod
    def load_images(char):
        if char in Monster3.images:
            return Monster3.images[char]

        images = {}
        count = 0
        file_fmt = '%s/Zombiefiles/%s/%s (%d).png'
        for action in Monster3.ACTIONS:
            action_images = []
            n = 0
            while True:
                n += 1
    ...
    def draw(self):
        ...
        # x,y = self.pos
        # Monster3.font.draw(x-40, y+50, self.action + str(round(self.time * 100) / 100))
